...
const PaymentForm = ({ paymentData, setShowPayment }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (amountValue <= 0 || amountValue > student.amountRemaining) {
      setError(
        "Amount must be greater than 0 and less than or equal to remaining amount.",
      );
  ...
  return (
    <div className="max-w-lg bg-white shadow-custom mx-auto md:p-6 p-2">
      <h2 className="text-2xl text-secondary text-center font-bold mb-4">
        Payment Form
      </h2>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          <label className="block">
            Due Date: {dueDate && "Due date not required"}
          </label>
          <input
            type="date"
            value={newDueDate}
        ...
        <div className="flex gap-2">
          {!error && receiptPhoto.isSaved ? (
            <button
              ...
              className="bg-green-500 min-w-36 disabled:opacity-50 grid place-items-center text-white px-4 py-2 rounded hover:bg-green-600 mr-2"
            >
              {submitLoader ? (
