...
export default function ImageFrameEditor({
  ...
  useEffect(() => {
    ...
    [
      ...
      ctx.beginPath();
      ctx.moveTo(x, y);
      ctx.lineTo(x + dx[0] * L, y + dx[1] * L);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(x, y);
      ctx.lineTo(x + dy[0] * L, y + dy[1] * L);
      ctx.stroke();
    });
    ctx.restore();
  ...
  const handleConfirm = () => {
    ...
    out.toBlob(
      (blob) => {
        const file = new File([blob], `framed_${Date.now()}.jpg`, {
          type: "image/jpeg",
  ...
  // ── Use original image, skip cropping entirely ───────────────────────────
  const handleUseOriginal = async () => {
    if (usingOriginal) return; // prevent double-clicks while converting
    setUsingOriginal(true);
    try {
      const res = await fetch(imageSrc);
      const blob = await res.blob();
      const ext = (blob.type && blob.type.split("/")[1]) || "jpg";
      const file = new File([blob], `original_${Date.now()}.${ext}`, {
        type: blob.type || "image/jpeg",
        lastModified: Date.now(),
      });
      onConfirm(file, URL.createObjectURL(blob));
    } catch (err) {
      console.error("Failed to use original image:", err);
  ...
  return (
    ...
    >
      ...
      >
        <p
          style={{ margin: 0, fontSize: 12, color: "#1d4ed8", lineHeight: 1.5 }}
        >
      ...
      >
        ...
        {/* Action buttons row — Use Original (secondary) + Apply Frame (primary) */}
        <div style={{ display: "flex", gap: 10 }}>
          <button
            type="button"
            onClick={handleUseOriginal}
            disabled={!imgLoaded || usingOriginal}
            title="Skip cropping and use the photo exactly as uploaded"
            style={{
              flex: "0 0 auto",
              background: "#ffffff",
              color: "#374151",
              ...
              fontWeight: 600,
              fontSize: 14,
              cursor: !imgLoaded || usingOriginal ? "default" : "pointer",
              opacity: !imgLoaded || usingOriginal ? 0.6 : 1,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              gap: 7,
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              fontSize: 15,
              cursor: usingOriginal ? "default" : "pointer",
              opacity: usingOriginal ? 0.6 : 1,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              gap: 8,
            ...
            onMouseEnter={(e) => {
              if (!usingOriginal) e.currentTarget.style.opacity = "0.88";
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.opacity = usingOriginal ? "0.6" : "1";
            }}
            onTouchStart={(e) => {
              if (!usingOriginal) e.currentTarget.style.opacity = "0.82";
            }}
            onTouchEnd={(e) => {
              e.currentTarget.style.opacity = usingOriginal ? "0.6" : "1";
            }}
